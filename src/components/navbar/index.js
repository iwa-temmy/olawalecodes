...
const Navbar = () => {
  ...
  return (
    <header className="navbar">
      ...
      <button className="nav-toggle" onClick={(e) => handleNavToggle(e)}>
        {opennav ? <AiIcons.AiOutlineClose /> : <GiIcons.GiHamburgerMenu />}
      </button>
      <nav>
        <ul className={opennav ? "navlinks sr-only" : "navlinks"}>
          ...
          <li
            className={splitLocation[1] === "work" ? "active" : ""}
            onClick={handleNavToggle}
          >
            <Link to="/work">Work</Link>
          </li>
          <li className={splitLocation[1] === "resume" ? "active" : ""}>
            <a href={myCV} download="olawale_IwaloyeCV.pdf">
              Resume
            </a>
          </li>
        </ul>
